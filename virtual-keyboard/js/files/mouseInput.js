  ...
  key.addEventListener('click', function (event) {
    ...
    switch (key.dataset.code) {
      ...
      case 'Delete': {
        ...
        textarea.selectionStart = textarea.selectionEnd = caretPosition.end;
        break;
      }
      case 'Tab': {
        textarea.value = text.slice(0, caretPosition.start) + '\t' + text.slice(caretPosition.end);
      ...
      case 'ArrowLeft': {
        if (leftShiftKey.classList.contains('active') || rightShiftKey.classList.contains('active')) {
          textarea.selectionStart !== 0 ? textarea.selectionStart = caretPosition.start - 1 : textarea.selectionStart = caretPosition.start;
        } else {
          textarea.selectionStart = textarea.selectionEnd = caretPosition.start - 1;
        }
        break; 
      ...
      case 'ArrowRight': {
        if (leftShiftKey.classList.contains('active') || rightShiftKey.classList.contains('active')) {
          textarea.selectionEnd !== text.length ? textarea.selectionEnd = caretPosition.end + 1 : textarea.selectionEnd = caretPosition.end;
        } else {
          textarea.selectionStart = textarea.selectionEnd = caretPosition.start + 1;
        }
        break;
